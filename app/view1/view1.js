      ...
      return $q(function(resolve, reject){
        resolve(
          [
            ...
            ["fluiten", "floot", "floten", "gefloten", "свистать"],
...
//TODO: 1s delay before colouring.
//TODO: stats from start to end
//TODO: stats on particular word for the choice selection
.directive('checkingInput', function() {
  return {
    scope: {
      caption : '@caption',
      expected : '=text'
    },
    template: '<input type="text" placeholder="{{caption}}" ng-model="userInput" ng-style="{color: expected == userInput ? \'green\' : \'red\'}"/>'
  }
})
...
.directive('trans', function() {
  return {
    ...
    template: '<span ng-show="text && !visible" ng-click="visible=true" title="{{text}}">?</span><span ng-show="visible">{{text}}</span>'
  }
})

.controller('View1Ctrl', function($scope, WordList) {
  WordList.fetch().then(function(ok){
    $scope.list = ok;
  });
});